...
// --------------------------------------------------------------------------
// Global vars (defined in VisualizerDefs.h)
			...
			case WAIT_OBJECT_0:
				__try {
					...
					for (int i = 0; i < N; i++) {
						...
						// set peak value
						if (fft.in[i].r > anim.peak) anim.peak = fft.in[i].r;
					}
					dwCount++;
				}
				__finally {
	...
	while (t < HALFN && num < HALFN) {
		...
		if (y > prev) {
			if (display.yscaling > 0)
				dataVals.points[num][1] = prev * (1 + 0.01f * anim.rateAdjust);
			else dataVals.points[num][1] = y;
		}
		else {
			if (display.yscaling > 0)
				dataVals.points[num][1] = prev * (1 - 0.001f * anim.rateAdjust);
			else dataVals.points[num][1] = y;
		}
...
void GetIndices() {
	...
	if (display.mainShape == 0) {
		...
		// set indices for main shape
		while (i < MAXVERTS - display.srevSplits - 2) {
			if (ii + 5 < MAXINDS) {
				dataVals.indices[ii] = i;
				dataVals.indices[ii + 1] = i + 1;
				dataVals.indices[ii + 2] = i + display.srevSplits + 1;
				dataVals.indices[ii + 3] = i + display.srevSplits + 2;
				dataVals.indices[ii + 4] = i + display.srevSplits + 1;
				dataVals.indices[ii + 5] = i + 1;
		...
		while (ii + 2 < MAXINDS) {
			dataVals.indices[ii] = i;
			dataVals.indices[ii + 1] = i;
			dataVals.indices[ii + 2] = i;
			ii += 3;
		}
	}
...
float *ColourFunction(float x, float v, float absy) {
	...
	if (display.colours == 1) {
		colours[0] = 0.2 + 0.5f * (1 + cosf(x + v));
		colours[1] = 0.2 + 0.5f * (1 + sinf(3.f * x + v));
		colours[2] = 0.2 + absy;
		return colours;
	}
...
void SetScene(int s) {
	...
	if (s == 2) {
		...
		display.scene = 2; display.colours = 2;
		display.shape = 0; display.mainShape = 0;
		display.skySphereSplits = 6;
		display.srevInc = PI / 6;
		display.srevMax = display.srevInc * 2;
		display.srevSplits = 2;
		display.tessInner = 1; display.tessOuter = 1;
	...
	// scene 3
		...
		SetFlags(false, false, flags.antialiasing, false, true, false, true, false, false, false, false, false);
		camera.camR = 20;
		SetBSpline(3, 0.005f);
		display.xInc = 0.1; display.yAdjust = 1.f;
		display.scene = 3; display.colours = 2;
		display.shape = 1; display.mainShape = 0;
		display.skySphereSplits = 12;
...
void UpdateCamera(MyUniforms *uniforms) {
	...
	float fov = camera.fov + anim.peak * sin(anim.colourAdjust) * PI / 36; // TODO smooth peak, animation val
...
void UpdateAnimations() {
	...
	if (flags.peakRotateP)
		MoveCamera(0.001f * anim.rateAdjust * anim.peak, 0);
	if (flags.peakRotateT) 
		MoveCamera(0, 0.00075f * anim.rateAdjust * anim.peak);
